...
async function createSubscription(customer) {
  try {
    const subscriptionObj = {
      ...
      items: [{ price: staqConfig.get("stripeTrialPriceId") }],
    };

    if (staqConfig.get("useTrial")) {
      subscriptionObj.trial_period_days =
        staqConfig.get("stripeTrialPeriodDays") || 14;
    }
...
async function createCustomer(data, context, stripe) {
  try {
    ...
    const subscription =
      staqConfig.get("paymentType") === "subscription"
